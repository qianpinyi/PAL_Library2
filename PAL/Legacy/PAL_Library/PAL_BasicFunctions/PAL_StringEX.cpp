...
namespace PAL::Legacy
{
	...
	class stringUTF8
	{
		...
		public:
			...
			int find(const stringUTF8 &str)
			{
				...
				{
					bool flag=1;
					for (size_t j=0;j<str.strUTF8.size();++j)
	...
	double strTOdb(const std::string &str)
	{
		if (str.empty()) return 0;
		double x=0;
		for (size_t i=str[0]=='-',flag=0;i<str.length();++i)
			if (str[i]=='.') flag=1;
	...
	std::string ullTOpadic(unsigned long long x,bool isA=1,unsigned char p=16,unsigned int w=1)
	{
		std::string re;
		std::stack <char> sta;
		while (x)
...
//	std::string ReplaceStrInStr(const std::string &str,const std::string &s1,const std::string &s2)
//	{
//		std::string re;
//		for (size_t i=0;i<str.length();)
//			if (size_t p=str.find(i,s1);p!=str.npos)
//			{
//				re+=str.substr(i,p-i)+s2;
//				i=p+s1.length();
//			}
//			else
//			{
//				re+=str.substr(i,str.length()-i);
//				break;
//			}
//		return re;
//	}
	...
	{
		if (str.empty()) return "";
		for (long long i=(long long)str.length()-1;i>=0;--i)
			switch (str[i])
			{
				case '\n':break;//??
				case '\r':break;//??
	...
	std::wstring DeleteEndBlank(const std::wstring &wstr)
	{
		...
		for (long long i=(long long)wstr.length()-1;i>=0;--i)
			switch (wstr[i])
			{
				case L'\n':break;//??
				case L'\r':break;//??
	...
	std::string GetAftername(const std::string &str)
	{
		...
		for (long long i=(long long)str.length()-1;i>=0;--i)
			switch (str[i])
			{
				case '.':	return str.substr(i,str.length()-i);
				case '/':	return "";
				case '\\':	return "";
			}
		return "";
	...
	std::string ReplaceAftername(const std::string &str,const std::string &tar)
	{
		if (str.empty()) return "";
		for (long long i=(long long)str.length()-1;i>=0;--i)
			switch (str[i])
			{
				case '.':	return str.substr(0,i)+tar;
				case '/':	return str;
				case '\\':	return str;
			}
		return str;
	...
	//	if (p==str.length()-1) return "";
	//	else if (p==str.npos) return str;
	//	else return str.substr(p+1,str.length()-p-1);
	//}
	//
	...
	std::wstring GetLastAfterBackSlash(const std::wstring &str)
	{
		if (str.empty()) return L"";
		size_t p=str.rfind(PLATFORM_PATHSEPERATORW,str.length()-1);
		if (p==str.length()-1) return L"";
		else if (p==str.npos) return str;
		else return str.substr(p+1,str.length()-p-1);
	}

	std::wstring GetPreviousBeforeBackSlash(const std::wstring &str)
	{
		if (str.empty()) return L"";
		size_t p=str.rfind(PLATFORM_PATHSEPERATORW,str.length()-1);
		if (p==str.npos||p==0) return L"";
		return str.substr(0,p);
	}
	...
	{
		if (str.empty()) return "";
		size_t p=str.rfind(PLATFORM_PATHSEPERATOR,str.length()-1);
		if (p==str.length()-1) return "";
		else if (p==str.npos) return str;
		else return str.substr(p+1,str.length()-p-1);
	}
	...
	std::wstring GetLastAfterSlashOrBackSlash(const std::wstring &str)
	{
		if (str.empty()) return L"";
	...
	std::string GetLastAfterSlashOrBackSlash(const std::string &str)
	{
		if (str.empty()) return "";
		for (long long i=str.length()-1;i>=0;--i)
			if (str[i]=='/'||str[i]=='\\')
	...
	std::wstring GetPreviousBeforeSlashOrBackSlash(const std::wstring &str)
	{
		if (str.empty()) return L"";
		for (long long i=str.length()-1;i>=0;--i)
			if (str[i]==L'/'||str[i]==L'\\')
	...
	std::string GetPreviousBeforeSlashOrBackSlash(const std::string &str)
	{
		if (str.empty()) return "";
		for (long long i=str.length()-1;i>=0;--i)
			if (str[i]=='/'||str[i]=='\\')
	...
	PAL_DS::Doublet <std::string,std::string> DivideInLastBackSlash(const std::string &str)
	{
		if (str.empty()) return {"",""};
		size_t p=str.rfind(PLATFORM_PATHSEPERATOR,str.length()-1);
		if (p==str.npos||p==0) return {"",str};
		return {str.substr(0,p),str.substr(p+1,str.length()-p-1)};
	}
	...
	std::string GetRandStr(int w,const std::string &charset)
	{
		std::string re;
		for (int i=1;i<=w;++i)
			re+=charset[rand()%charset.size()];
	...
	bool HaveSamePrefix(const std::string str,const std::string &prefix)
	{
		if (str.length()<prefix.length())
			return false;
		for (size_t pos=0;pos<str.length()&&pos<prefix.length();++pos)
	...
	std::string GetSubStrBeforeCntChar(const std::string &str,char ch,size_t cnt=1)
	{
		size_t pos=0,t=0;
		while (t<cnt&&pos<str.length())
		{
		...
		if (pos==0||pos==1)
			return "";
		else if (pos!=str.npos&&pos<str.length())
	...
	std::vector <std::string> DivideStringByString(const std::string &str,const std::string &s2)//Can be optimized by KMP?
	{
		using namespace std;
		vector <string> re;
		size_t p=s2.length();
	...
	std::vector <std::string> DivideStringByBlank(const std::string &str)
	{
		using namespace std;
		vector <string> re;
		string s;
	...
	std::string SpliceStringByChar(const std::vector <std::string> &vec,char ch)
	{
		std::string re;
		bool flag=0;
	...
	inline std::string SpliceStringByLine(const std::vector <std::string> &vec)
	{return SpliceStringByChar(vec,'\n');}

	inline std::string SpliceStringByString(const std::vector <std::string> &vec,const std::string &str)
	{
		std::string re;
		bool flag=0;
		for (const auto &vp:vec)
	...
	inline std::wstring SpliceStringByString(const std::vector <std::wstring> &vec,const std::wstring &str)
	{
		std::wstring re;
		bool flag=0;
		for (const auto &vp:vec)
	...
	std::string FilterStringChars(const std::string &str,const std::string &charset)
	{
		std::string re;
		std::set <char> se;
		for (auto sp:charset)
			se.insert(sp);
		for (auto sp:str)
			if (se.find(sp)!=se.end())
				re+=sp;
		return re;
	}

	std::string FilterStringChars_Reversed(const std::string &str,const std::string &charset)
	{
		std::string re;
		std::set <char> se;
		for (auto sp:charset)
	...
	template <typename T> std::vector <T> OperateForAll(std::vector <T> vec,void(*func)(T&))
	{
		for (auto &vp:vec)
			func(vp);
		return vec;
	...
	inline int FileWriteString(std::ofstream &fout,const std::string &str)
	{
	...
	inline std::string FileReadString(std::ifstream &fin)
	{
		unsigned int len;
		fin.read((char*)&len,4);
		if (len==0) return "";
	...
	bool SortComp_WithNum(const std::string &a,const std::string &b)
	{
		...
		while (p<a.length()&&q<b.length())
		{
			...
			{
				char cha=Atoa(a[p]),chb=Atoa(b[q]);
				if (cha==chb)
					++p,++q;
				else if (cha>0&&chb>0)
					return cha<chb;
		...
		return p==a.length()&&q!=b.length();
	}

	long long GetAndCheckStringNaturalNumber(const std::string &str)
	{
		if (str=="") return -1;
		long long x=0;
		for (auto sp:str)
	...
	std::string BinToStr(const void *data,unsigned size,bool IsW=1)
	{
		static const char HexStr[2][17]{"0123456789abcdef","0123456789ABCDEF"};
		std::string re;
		for (unsigned i=0;i<size;++i)
			re+=HexStr[IsW][(*((unsigned char*)data+i)&0xf0)>>4],
			re+=HexStr[IsW][(*((unsigned char*)data+i)&0x0f)];
		return re;
	}

	size_t StrToBin(const std::string &str,void *data)
	{
