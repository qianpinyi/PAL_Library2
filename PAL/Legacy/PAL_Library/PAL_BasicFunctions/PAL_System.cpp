...
namespace PAL::Legacy
{
	namespace
	{
	#ifdef PAL_AUTOINIT_VTERM
		class AutoInitVtermClass
		{
			public:
				AutoInitVtermClass()
				{
	...
	#endif
	#ifdef PAL_AUTOINIT_U8TERM
		class AutoInitU8TermClass
		{
			public:
				AutoInitU8TermClass()
				{
	...
	std::string GetSystem32DirectoryPath()
	{
		...
		if (GetSystemDirectoryW(buffer,1024))
			return Charset::UnicodeToUtf8(buffer);
		else return "";
	}
	...
	{return IsFileExsit(Charset::Utf8ToUnicode(path),filter);}
	
	bool CreateDirectoryUTF8(const std::string &path)
	{return CreateDirectoryW(Charset::Utf8ToUnicode(path).c_str(),NULL);}

	bool CreateDirectoryR(const std::wstring &path)
	{
		if (path==L"")
			return true;
		if (!IsFileExsit(path,1))
		{
			if (!CreateDirectoryR(GetPreviousBeforeSlashOrBackSlash(path)))
				return false;
	...
	{return CreateDirectoryR(Charset::Utf8ToUnicode(path));}
	
	bool CopyFileUtf8(const std::string &src,const std::string &dst,bool override=false)
	{return CopyFileW(Charset::Utf8ToUnicode(src).c_str(),Charset::Utf8ToUnicode(dst).c_str(),!override);}
	
	bool MoveFileUtf8(const std::string &src,const std::string &dst)
	{return MoveFileW(Charset::Utf8ToUnicode(src).c_str(),Charset::Utf8ToUnicode(dst).c_str());}
	
	bool DeleteFileUtf8(const std::string &path)
	{return DeleteFileW(Charset::Utf8ToUnicode(path).c_str());}
	
	bool RemoveDirectoryUtf8(const std::string &path)
	{return RemoveDirectoryW(Charset::Utf8ToUnicode(path).c_str());}
}
