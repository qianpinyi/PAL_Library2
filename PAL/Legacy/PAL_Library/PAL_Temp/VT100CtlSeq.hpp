...
namespace PAL::Legacy::Temp::VT100CtlSeq
{
	namespace ControlSequenceName
	{
		enum ControlSequenceType
		{
			...
			NONE=0,//When not a conseq, raw text will be saved in text
	...
	template <class T> class ControlSequenceParser
	{
		protected:
			void (*Func)(ControlSequenceParser<T>&,const ControlSequenceCode&)=nullptr;
			T Data;
			std::string LastSeq;

			void ConsumeText(std::string &str)
			{
				if (str=="")
					return;
				ControlSequenceCode code;
				code.type=ControlSequenceName::NONE;
				code.text=str;
				Func(*this,code);
				str="";
			}
		...
		public:
			...
			bool Empty() const
			{
				return LastSeq.empty();
			}
			
			void Input(const char *s,const char *e=nullptr)
			{
				using namespace ControlSequenceName;
				std::string str=LastSeq;
				LastSeq="";
				if (e==nullptr)
					str.append(s);
				else str.append(s,e-s);
				std::string ss;
				for (const char *s=str.c_str();*s;)
      				if (*s=='\e')
					{
						ConsumeText(ss);
						auto code=ParseControlSequence(s);
						if (InThisSet(code(),PARSE_REACHEND))//??
						{
							LastSeq=s;
							return;
						}
			...
			ControlSequenceParser& operator << (const char *str)
			{
				Input(str);
				return *this;
			}
			
			ControlSequenceParser& operator << (const std::string &str)
			{
				Input(str);
				return *this;
			}
			
			ControlSequenceParser(decltype(Func) func,const T &data):Func(func),Data(data)
			{
				assert(func!=nullptr);
			}
	};

#ifdef PAL_BASICFUNCTIONS_COLOR_CPP
